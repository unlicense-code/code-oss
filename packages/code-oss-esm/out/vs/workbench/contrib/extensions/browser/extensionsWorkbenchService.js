...
let Extension = class Extension {
    constructor(stateProvider, runtimeStateProvider, server, local, _gallery, resourceExtensionInfo, galleryService, telemetryService, logService, fileService, productService) {
        ...
        this.enablementState = 9 /* EnablementState.EnabledGlobally */;
...
let ExtensionsWorkbenchService = class ExtensionsWorkbenchService extends Disposable {
    ...
    computeExtensionsNotifications() {
        ...
        const invalidExtensions = this.local.filter(e => e.enablementState === 5 /* EnablementState.DisabledByInvalidExtension */ && !e.isWorkspaceScoped);
    ...
    async install(arg, installOptions = {}, progressLocation) {
        ...
        if (installOptions.enable) {
            if (extension.enablementState === 8 /* EnablementState.DisabledWorkspace */ || extension.enablementState === 7 /* EnablementState.DisabledGlobally */) {
                if (installOptions.justification) {
                    const result = await this.dialogService.confirm({
                ...
                await this.setEnablement(extension, extension.enablementState === 8 /* EnablementState.DisabledWorkspace */ ? 10 /* EnablementState.EnabledWorkspace */ : 9 /* EnablementState.EnabledGlobally */);
            }
    ...
    promptAndSetEnablement(extensions, enablementState) {
        const enable = enablementState === 9 /* EnablementState.EnabledGlobally */ || enablementState === 10 /* EnablementState.EnabledWorkspace */;
        if (enable) {
    ...
    getExtensionsRecursively(extensions, installed, enablementState, options, checked = []) {
        ...
        if (toCheck.length) {
            ...
            const extensionsToEanbleOrDisable = installed.filter(i => {
                ...
                const enable = enablementState === 9 /* EnablementState.EnabledGlobally */ || enablementState === 10 /* EnablementState.EnabledWorkspace */;
                const isExtensionEnabled = i.enablementState === 9 /* EnablementState.EnabledGlobally */ || i.enablementState === 10 /* EnablementState.EnabledWorkspace */;
    ...
    async doSetEnablement(extensions, enablementState) {
        ...
        for (let i = 0; i < changed.length; i++) {
            if (changed[i]) {
                ...
                */
                this.telemetryService.publicLog(enablementState === 9 /* EnablementState.EnabledGlobally */ || enablementState === 10 /* EnablementState.EnabledWorkspace */ ? 'extension:enable' : 'extension:disable', extensions[i].telemetryData);
            }
        }
        return changed;
    }
